(function(global, factory) {
	...
	//#endregion
	//#region node_modules/.pnpm/ini@7.0.0/node_modules/ini/lib/ini.js
	...
	async function extractCore(core) {
		const localUrl = `./cores/${core}_libretro.zip`;
		const cdnUrl = `${cdnBaseUrl}/${coreRepo}@${coreVersion}/${coreDirectory}/${core}_libretro.zip`;
		let url = cdnUrl;
		try {
			const headCheck = await fetch(localUrl, { method: 'HEAD' });
			if (headCheck.ok) url = localUrl;
		} catch(e) {}
		...
		*/
		...
		async updateRom() {
			...
			if (this.multiDisc) {
				...
				this.multiDiscRom = /* @__PURE__ */ new Map();
		...
		async createResolvableFile(file) {
			const { resolveRom } = this.nostalgistOptions;
			const rom = await ResolvableFile.create(typeof file === "string" ? {
				raw: file,
				signal: this.signal,
				urlResolver: () => resolveRom(file, this.nostalgistOptions)
			} : {
				raw: file,
				signal: this.signal
			});
	...
	var Emulator = class {
		...
		async saveState() {
			...
			return {
				state,
				thumbnail,
				...multiDiscIndex !== void 0 && { multiDiscIndex }
			};
		}
		async screenshot() {
		...
		async switchDisc(index) {
			const { multiDiscRom } = this.options;
			if (!multiDiscRom) return;
			...
			if (requestedRom) {
				...
				if (!requestedResolvableFile) return;
				await this.fs.writeFile(path.join(EmulatorFileSystem.contentDirectory, requestedResolvableFile.name), requestedResolvableFile);
				multiDiscRom.delete(index);
			}
			this.sendCommand("DISK_EJECT_TOGGLE");
			while (delta !== 0) if (delta > 0) {
		...
		runMain() {
			...
			if (this.options.multiDisc) this.options.multiDiscIndex = 1;
	...
	return class Nostalgist {
		...
		*/
		async switchDisc(index) {
			return await this.getEmulator().switchDisc(index);
		}
		/**
