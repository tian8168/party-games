...
new_build_level = '''
    function buildContraLevel() {
      ...
      // Reset dynamic spawners and entities
...
spawner_code = '''
      ...
      STATE.contra.platforms.forEach(plat => {
        if (plat.type === 'bridge' && plat.timer > 0) {
          plat.timer--;
          if (plat.timer === 0) {
            plat.active = false; // Mark for removal
            for (let i = 0; i < 15; i++) {
...
trigger_bridge_code = '''
          if (p.vy >= 0 && pFootY >= plat.y && (pFootY - p.vy <= plat.y + 16 || pFootY <= plat.y + 12)) {
            ...
            if (plat.type === 'bridge' && plat.timer === -1) {
              plat.timer = 60; // 1 second to explode
            }
          }
'''
